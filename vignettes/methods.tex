...
($H^2$) for GBS data with high error rate. Our hidden states are the space of a
3-tuple of alleles: genotyped parents transmitted allele, genotyped parents
untransmitted allele, and the offspring's allele from the non-genotyped parent.
Observed values are the emitted genotypes from these states. There are $2^3$
possible k-tuples for the duo HMM.
...
Switches between internal state don't matter?

Transitions of haplotype k-tuples.
